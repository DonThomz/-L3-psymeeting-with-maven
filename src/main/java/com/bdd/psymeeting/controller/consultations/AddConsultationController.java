...
import com.bdd.psymeeting.model.tools.TransitionEffect;
...
public class AddConsultationController implements Initializable, InitController {
    ...
    public void initListeners() {
        ...
        preloadPatientsComboBox.setOnAction(event -> {
            ...
            last_name_field.setText(preloadPatientsComboBox.getValue().getLastName());
    ...
    private boolean addPatient2ArrayList() {
        if (userExist()) { // check if user exist <==> email exit in database
            ...
            if (lastPrimaryKeyPatient != -1 && lastPrimaryKeyUser != -1) {
                ...
                // add new user
                users.add(new User(lastPrimaryKeyUser + lastIDUser, email_field.getText(), tmpPatient.getPatientId(), true));
    ...
    private void attachPatients() { // attach patient to the right side
        ...
        String text = "Patient"
                ...
                + " " + patients.get(patients.size() - 1).getLastName();
